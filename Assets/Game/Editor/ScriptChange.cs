...
{
    static string namespaceName = "BoringWorld";
    ...
    static void OnWillCreateAsset(string path)
    {
        ...
        if (splitArgs[splitArgs.Length - 1].Equals("cs") && str.Contains("Game/"))
        {
            //Debug.Log("导入的是脚本");
    ...
    private static void ParseAndChangeScript(string path)
    {
        ...
        if (!str.Contains("namespace"))
        {
            if (!string.IsNullOrEmpty(namespaceName))
            {
                ...
                for (int i = 0; i < extraStrs.Length; i++)
                {
                    extraStr +=  i == extraStrs.Length - 1 ? extraStrs[i] : "\t" + extraStrs[i] + "\r\n";
                }

                newStr += "namespace " + namespaceName + "\r\n{\r\n" + extraStr + "}";
